# This is a bot developed by Leonard Cucos @ www.leonardcucos.com and published under the GNU General Public License 3.0. Feel free to run, study, share, and modify
# the code. You can add extra features (GUI, TA, etc.) or contact me if you need my help at https://leonardcucos.com/contact
# Enjoy!
...
class Bitmex:  # define new class called Bitmex , which defines algorithm framework and parameters with different methods
    ...
    def get_instrument_query(self, symbol):  # retrieve prices for mark and last price
        json_, adapter = self._client.Instrument.Instrument_get(symbol=symbol, reverse=True, count=1).result()
        inst = InstrumentQuery(json_[0])  # InstrumentQuery is class we have defined above
    ...
    def get_dataframe(self, symbol, timeframe):
        ...
        json_, adapter = self._client.Trade.Trade_getBucketed(
            binSize=timeframe,
            symbol=symbol,
            count=750,
            startTime=start,
    ...
    def _enter_market(self, symbol, size,
        ...
        json_, adapter = order.result()
        order = Order(json_)  # Order is class we have defined above
    ...
    def _calc_targets(self, entry_price, side, target_offset=50,
                      stop_offset=50):  # calculate targets for stops and limits
        if side == 'buy':
    ...
    def enter_bracket(self, symbol, size, side='buy', target_offset=50, stop_offset=50):  # create bracket order
    ...
    def check_exit_fill(self):
        json_, adapter = self._client.Order.Order_getOrders(filter=json.dumps({"open": True})).result()
...
class Trader:  # define another new class called Trader, where trading happens
    ...
    def __init__(self, symbol, model, size, timeframe='1d',
                 # again, first, need to define our _init_ instance before our algo methods
                 target_offset=50, stop_offset=50):
        self.symbol = symbol
        self.model = model
        self.size = size
...
if __name__ == '__main__':  # where we call our Trader class defined above, in main
    model = EnsembleModel()
    trader = Trader('XBTUSD', model, 10, '5m',
                    target_offset=50, stop_offset=50)
    try:
        trader.run()
    except:
